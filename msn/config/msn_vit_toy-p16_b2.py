# This config is used for debugging.
custom_imports = dict(
    imports=[
        'model.msn_pipeline',
        'model.msn',
...
rand_view_pipeline = [dict(type='MSNRandTransform')]
focal_view_pipeline = [dict(type='MSNFocalTransform')]
...
# model
embed_dim = 128
hidden_dim = 256  # Used in model neck
output_dim = 128
...
model = dict(
    ...
    init_cfg=dict(
        type='Pretrained',
        checkpoint='/home/dqwang/scratch/likaixin/work_dir/init_mmlab.pth'
    )
)
...
optimizer = dict(type='AdamW', lr=lr, paramwise_options={
    '.*(bias|bn|norm).*': dict(WD_exclude=True, weight_decay=0),  # len(p.shape)=1怎么办？
